...
import './interfaces/IAthleteXRouter.sol';
...
contract AthleteXRouter is IAthleteXRouter {
    ...
    function _addLiquidity(
        ...
        if (IAthleteXFactory(factory).getPair(tokenA, tokenB) == address(0)) {
            IAthleteXFactory(factory).createPair(tokenA, tokenB);
        }
        (uint reserveA, uint reserveB) = AthleteXLibrary.getReserves(factory, tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint amountBOptimal = AthleteXLibrary.quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'AthleteXRouter: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = AthleteXLibrary.quote(amountBDesired, reserveB, reserveA);
                assert(amountAOptimal <= amountADesired);
                require(amountAOptimal >= amountAMin, 'AthleteXRouter: INSUFFICIENT_A_AMOUNT');
    ...
    function addLiquidity(
        ...
        address pair = AthleteXLibrary.pairFor(factory, tokenA, tokenB);
    ...
    function removeLiquidity(
        ...
        address pair = AthleteXLibrary.pairFor(factory, tokenA, tokenB);
        IAthleteXPair(pair).transferFrom(msg.sender, pair, liquidity); // send liquidity to pair
        (uint amount0, uint amount1) = IAthleteXPair(pair).burn(to);
        (address token0,) = AthleteXLibrary.sortTokens(tokenA, tokenB);
        (amountA, amountB) = tokenA == token0 ? (amount0, amount1) : (amount1, amount0);
        require(amountA >= amountAMin, 'AthleteXRouter: INSUFFICIENT_A_AMOUNT');
        require(amountB >= amountBMin, 'AthleteXRouter: INSUFFICIENT_B_AMOUNT');
    }
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            (address token0,) = AthleteXLibrary.sortTokens(input, output);
            uint amountOut = amounts[i + 1];
            (uint amount0Out, uint amount1Out) = input == token0 ? (uint(0), amountOut) : (amountOut, uint(0));
            address to = i < path.length - 2 ? AthleteXLibrary.pairFor(factory, output, path[i + 2]) : _to;
            IAthleteXPair(AthleteXLibrary.pairFor(factory, input, output)).swap(
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = AthleteXLibrary.sortTokens(input, output);
            IAthleteXPair pair = IAthleteXPair(AthleteXLibrary.pairFor(factory, input, output));
            uint amountInput;
            uint amountOutput;
            ...
            amountOutput = AthleteXLibrary.getAmountOut(amountInput, reserveInput, reserveOutput);
            }
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        require(
            ...
            'AthleteXRouter: INSUFFICIENT_OUTPUT_AMOUNT'
        );
    }
    ...
    function quote(uint amountA, uint reserveA, uint reserveB) public pure virtual override returns (uint amountB) {
        return AthleteXLibrary.quote(amountA, reserveA, reserveB);
    }
    ...
    {
        return AthleteXLibrary.getAmountOut(amountIn, reserveIn, reserveOut);
    }
    ...
    {
        return AthleteXLibrary.getAmountIn(amountOut, reserveIn, reserveOut);
    }
    ...
    function getAmountsOut(uint amountIn, address[] memory path)
        ...
        returns (uint[] memory amounts)
    {
        return AthleteXLibrary.getAmountsOut(factory, amountIn, path);
    }
    ...
    function getAmountsIn(uint amountOut, address[] memory path)
        ...
        returns (uint[] memory amounts)
    {
        return AthleteXLibrary.getAmountsIn(factory, amountOut, path);
    }
}
